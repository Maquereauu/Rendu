...
bool GCTexture::Initialize(std::string fileName, GCGraphics* pGraphics)
{
    ...
    DirectX::CreateDDSTextureFromFile12(pGraphics->GetRender()->Getmd3dDevice(), pGraphics->GetRender()->GetCommandList(), Filename.c_str(), &m_pTextureBuffer, &m_pUploadTexture);
